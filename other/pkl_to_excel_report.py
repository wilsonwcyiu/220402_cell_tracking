...
def write_to_excel(group_truth_track_dict, method_name_track_data_dict):
    ...
    sheet = wb.create_sheet("sheet1") #inserts at first position

    # add header
    cell = sheet.cell(row = 1, column = 1, value = "series")
    cell.font = bold_font
    sheet.column_dimensions[cell.column_letter].width = 5

    cell = sheet.cell(row = 1, column = 2, value = "method_name")
    cell.font = bold_font
    ...
    data_start_col = 3
    frame_num_cnt = 1
    for col_num in range(data_start_col, 135):
        cell = sheet.cell(row = 1, column = col_num, value = frame_num_cnt)
        frame_num_cnt += 1
        cell.font = bold_font
        sheet.column_dimensions[cell.column_letter].width = 3
    ...
    # cell = sheet.cell(row = 4, column = 2, value = 42)
    # cell.fill = yellow_fill

    #save the workbook
    wb.save(filename = "D:/viterbi linkage/dataset/save_directory_enhancement/comparison_report_excel/example.xlsx")



def convert_dict_key(input_track_dict, total_frame):
    series_cell_id_track_list_dict = defaultdict(dict)
    for series, track_tuple_list_list in input_track_dict.items():
        series_cell_id_track_list_dict[series] = {}
        track_tuple_list_list.sort()
        for track_tuple_list in track_tuple_list_list:
            cell_id = track_tuple_list[0]

            track_sequence_list = [None] * (total_frame + 1) # +1 as frame_num is bring used as list_idx
            for track_tuple in track_tuple_list:
                node_idx = track_tuple[0]
                frame_num = track_tuple[1] + 1
                track_sequence_list[frame_num] = node_idx

            series_cell_id_track_list_dict[series][cell_id] = track_sequence_list

    return series_cell_id_track_list_dict
...
class ProbabilityData():
    ...
    def __init__(self, series, frame_num, from_node_idx, to_node_idx, probability):
        self.series        = series
...
def compare_probability_data(probability_data_1, probability_data_2):
    if probability_data_1.probability < probability_data_2.probability:            return -1
    elif probability_data_1.probability >= probability_data_2.probability:          return 1
