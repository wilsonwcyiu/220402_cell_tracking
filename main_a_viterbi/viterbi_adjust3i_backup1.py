...
def main():
    ...
    is_active_min_probability_to_connect: bool = False

    hyper_para: HyperPara = HyperPara(strategy_enum, merge_threshold, minimum_track_length, cut_threshold, is_do_post_adjustment, is_active_min_probability_to_connect)
...
def execute_cell_tracking_task(frame_num_prof_matrix_dict: dict, hyper_para):
    ...
    frame_num_node_idx_cell_occupation_list_list_dict = \
        _process_and_find_best_cell_track(all_cell_id_track_list_dict,
                                          ...
                                          cell_id_frame_num_node_idx_best_index_list_dict_dict,
                                          cell_id_frame_num_node_idx_best_value_list_dict_dict,
                                          hyper_para.is_active_min_probability_to_connect,
    ...
    for frame_num in range(second_frame_num, last_frame_num):
        ...
        for cell_row_idx in range(frame_num_prof_matrix_dict[frame_num].shape[0]):  #skip all nodes which are already passed
            ...
            to_handle_cell_id_list: list = [CellId(frame_num, cell_row_idx)]
...
def _process_and_find_best_cell_track(existing_cell_idx_track_list_dict,
                                      ...
                                      cell_id_frame_num_node_idx_best_index_list_dict_dict,
                                      cell_id_frame_num_node_idx_best_value_list_dict_dict,
                                      is_active_min_probability_to_connect: bool,
    ...
    while len(to_handle_cell_id_list) != 0:
        ...
        for handling_frame_num in range(second_frame, last_frame_num):
            ...
            index_ab_vec, value_ab_vec = derive_last_layer_each_node_best_track(handling_cell_id,
                                                                                ...
                                                                                cell_id_frame_num_node_idx_best_value_list_dict_dict,
                                                                                strategy_enum,
                                                                                is_active_min_probability_to_connect,
                                                                                cut_threshold,
        ...
        if handling_cell_id not in to_skip_cell_id_list:
            ...
            # cell_track_list_1 = _cut_single_track(cell_track_list, cut_threshold, frame_num_prof_matrix_dict)
...
def _cut_1(cell_idx_track_list_dict: dict, cut_threshold: float, frame_num_prof_matrix_dict: dict):
    short_track_list_dict: dict = {}

    for cell_id, track_tuple_list in cell_idx_track_list_dict.items():
        short_track_list = []
        for idx in range(len(cell_idx_track_list_dict[cell_id]) - 1):
            frame_idx = track_tuple_list[idx][1]
            frame_num: int = frame_idx + 1

            current_node = track_tuple_list[idx][0]
            next_node = track_tuple_list[idx + 1][0]

            weight_between_nodes = frame_num_prof_matrix_dict[frame_num][current_node][next_node]
            if (weight_between_nodes > cut_threshold):
                short_track_list.append(track_tuple_list[idx])
            else:
                tmp1 = copy.deepcopy(track_tuple_list[0: idx])
                short_track_list = tmp1
                break


        if (len(short_track_list) == len(track_tuple_list)-1):
            tmp = track_tuple_list[-1]
        else:
            tmp = track_tuple_list[len(short_track_list)]
    ...
    # cut_track_list_dict: dict = {}
    # for cell_id, track_tuple_list in cell_idx_track_list_dict.items():
    #     cut_track_list_dict[cell_id] = _cut_single_track(track_tuple_list, cut_threshold, frame_num_prof_matrix_dict)
    #
    # return cut_track_list_dict
...
def derive_last_layer_each_node_best_track(handling_cell_id,  # CellId
                                           ...
                                           strategy_enum,
                                           is_active_min_probability_to_connect: bool,
                                           min_probability_to_connect: float,
                                           cell_id_track_list_dict,
                                           existing_cell_idx_track_list_dict):
    ...
    for next_frame_node_idx in range(total_node_next_frame):
        ...
        for node_idx, node_connection_score in enumerate(node_connection_score_list):
            ...
            if is_active_min_probability_to_connect and node_connection_score <= min_probability_to_connect:
                continue
...
class HyperPara():
    def __init__(self,
        ...
        self.is_active_min_probability_to_connect = is_active_min_probability_to_connect
    ...
    def __eq__(self, other):
        if self.strategy_enum == other.routing_strategy_enum and \
