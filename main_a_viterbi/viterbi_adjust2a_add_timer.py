...
def _iteration(transition_group):
    ...
    for cell_track_list in store_dict.values():
        # if cell_track_list[0] in [(7, 0, -1), (8, 0, -1), (10, 0, -1)]:
        print("before cut", cell_track_list)
    print()

    short_Tracks = _cut(store_dict, 0.01, transition_group)


    for cell_track_list in short_Tracks.values():
        # if cell_track_list[0] in [(7, 0, -1), (8, 0, -1), (10, 0, -1)]:
        print("before post adj", cell_track_list)
    print()
    ...
    # for cell_track_list in all_tracks.values():
    #     # if cell_track_list[0] in [(4, 0, -1)]:
    #     print("after frame2 new cell", cell_track_list)
...
for serie in series:
    ...
    # for cell_track_list in all_tracks.values():
    #     if cell_track_list[0] in [(0, 0, -1), (1, 0, -1), (2, 0, -1), (3, 0, -1), (4, 0, -1), (5, 0, -1)]:
    ...
    # for cell_track_list in final_result:
    #     if cell_track_list[0] in [(4, 0, -1)]:
    #         print(cell_track_list)
    #
...
with open(save_dir + result_file_name + ".txt", 'w') as f:
    for s in series:
        ...
        f.write("\n")
        is_sort = False
        if is_sort: cell_track_list_list = sorted(viterbi_results_dict[s])
        else: cell_track_list_list = viterbi_results_dict[s]
        for cell_track_list in viterbi_results_dict[s]:
...
#     if cell_track_list[0] in ground_truth_cell_dict['S03'] :
#         print(cell_track_list)