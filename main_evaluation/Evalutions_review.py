...
def create_gt_idmap(ground_truth_track_tuple_list_list, predict_cell_track_tuple_list_list):
    gt_idmap_list = []
    for ground_truth_track_tuple_list in ground_truth_track_tuple_list_list:
        #get most similar predict_cell_track_tuple_list to ground truth predict_cell_track_tuple_list
        all_total_identical_track_point_list = []
        for predict_cell_track_tuple_list in predict_cell_track_tuple_list_list:
            #_adj remove third data in track tuple (previous node_idx)
            gt_track_adj_list = [ground_truth_track_tuple[0:2] for ground_truth_track_tuple in ground_truth_track_tuple_list]
            track_adj_list =    [predict_cell_track_tuple[0:2] for predict_cell_track_tuple in predict_cell_track_tuple_list]
            total_identical_track_point = len(set(gt_track_adj_list) & set(track_adj_list))
            all_total_identical_track_point_list.append(total_identical_track_point)

        idx_of_highest_identical_track_point: int = np.argmax(all_total_identical_track_point_list)
        if max(all_total_identical_track_point_list) > 0:
            a = ground_truth_track_tuple_list
            highest_identical_track_point_predict_track_list = predict_cell_track_tuple_list_list[idx_of_highest_identical_track_point]
            gt_idmap_list.append((a, highest_identical_track_point_predict_track_list))

    return gt_idmap_list
...
for name, result_dict in result_dicts:
    ...
    for serie in series:
        ...
        predict_cell_track_tuple_list_list = result_dict[identifier]
        ground_truth_track_tuple_list_list = gt_results_dict[identifier]

        gt_idmap_list = create_gt_idmap(ground_truth_track_tuple_list_list, predict_cell_track_tuple_list_list)
        prediction_idmap_list = create_estimate_idmap(ground_truth_track_tuple_list_list, predict_cell_track_tuple_list_list)

        total_frame = len(filelist)
        FIT_and_FIO_tuple_list = get_FIT_and_FIO(total_frame, gt_idmap_list, prediction_idmap_list)
        FIT_list = [i[0] / i[2] for i in FIT_and_FIO_tuple_list]
        FIO_list = [i[1] / i[2] for i in FIT_and_FIO_tuple_list]

        TP_list, OP_list = get_TP_and_OP(len(filelist), gt_idmap_list, prediction_idmap_list)

        tracking_performance_list.append([FIT_list, FIO_list, TP_list, OP_list, predict_cell_track_tuple_list_list])
...
methods = ['KDE method', 'DeLTA method', 'Hungarian method', 'Viterbi method']
